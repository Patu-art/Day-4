...
document.querySelectorAll(".video-toggle").forEach(btn=>btn.addEventListener("click",async()=>{const v=btn.parentElement.querySelector("video");if(v.paused){document.querySelectorAll("video").forEach(x=>{if(x!==v)x.pause()});try{await v.play();btn.textContent="PAUSE"}catch(e){}}else{v.pause();btn.textContent="PLAY"}}));
document.querySelectorAll("video").forEach(v=>v.addEventListener("pause",()=>{const b=v.parentElement.querySelector(".video-toggle");if(b)b.textContent="PLAY"}));
